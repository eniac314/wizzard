module Noise (perlin2D, noiseMat,Mat(..), fromMat, (§), matMap) where
...
data Mat a = Mat { mat :: Vec.Vector a
                 , row :: Int
                 , col :: Int
                 }

fromMat :: [a] ->  Mat a
fromMat xs = let l = length xs
                 r = floor.sqrt $ (fI l)
                 c = r 
             in Mat (Vec.fromList xs) r c

(§) :: Mat a -> (Int, Int) -> a
v § (r,c) = let (_,c') = (row v, col v)
            in (mat v) Vec.! (r*c' + c) 

matMap :: (a -> b) -> Mat a -> Mat b
matMap f m = let m' = Vec.map f (mat m)
             in m { mat = m' }
